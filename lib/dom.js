  ...
  for (var i = 0; i < results.snapshotLength; i++) {
    ...
    while (parentNode = parentNode.parentNode) {
      ...
      if (computedStyle.getPropertyValue('overflow') == 'hidden') {
        ...
        stayInBox = parentNodeRect.top    <= inspectionElement.top  &&
                    parentNodeRect.left   <= inspectionElement.left &&
                    parentNodeRect.bottom >= inspectionElement.top  &&
                    parentNodeRect.right  >= inspectionElement.left;
        break;
      }
    }
    if (!stayInBox) {
